function Isosurfaces( volume, isovalue, color_map )
{
    ...
    isovalue = KVS.Clamp( isovalue, smin, smax )*1.0;
        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
            for ( var x = 0; x < volume.resolution.x - 1; x++ )
            {
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 )
                {
					...
					var i2 = v2.x+v2.y*volume.resolution.x+v2.z*volume.resolution.x*volume.resolution.y;
					var i3 = v3.x+v3.y*volume.resolution.x+v3.z*volume.resolution.x*volume.resolution.y;
					var i4 = v4.x+v4.y*volume.resolution.x+v4.z*volume.resolution.x*volume.resolution.y;
					var i5 = v5.x+v5.y*volume.resolution.x+v5.z*volume.resolution.x*volume.resolution.y;

                    var v01 = interpolated_vertex( v0, v1, (isovalue - volume.values[ i0 ][0]) / (volume.values[ i1 ][0]-volume.values[ i0 ][0]));
                    var v23 = interpolated_vertex( v2, v3, (isovalue - volume.values[ i2 ][0]) / (volume.values[ i3 ][0]-volume.values[ i2 ][0]));
                    var v45 = interpolated_vertex( v4, v5, (isovalue - volume.values[ i4 ][0]) / (volume.values[ i5 ][0]-volume.values[ i4 ][0]));
    ...
    {
        var C0 = new THREE.Color().setHex( color_map[ isovalue ][1] );
        var C1 = new THREE.Color().setHex( color_map[ isovalue ][1] );
        var C2 = new THREE.Color().setHex( color_map[ isovalue ][1] );
    ...
    {
        return new THREE.Vector3().addVectors( v0.multiplyScalar(s), v1.multiplyScalar(1-s) );
    }
}
